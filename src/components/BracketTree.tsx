...
function TeamRow({
  ...
  if (!team) {
    return (
      <div className="flex items-center gap-1 px-1.5 opacity-35" style={{ height: rowH }}>
        <span className="text-xs w-4 text-center">—</span>
        <span className="text-[10px] text-white/55">待定</span>
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className={[
        ...
        isWinner ? 'bg-amber-400/20' : canClick ? 'hover:bg-white/10 active:bg-amber-400/12 cursor-pointer' : '',
      ].join(' ')}
      style={{ height: rowH }}
    >
      <span className="shrink-0 text-center leading-none"
        style={{ fontSize: '0.95rem', width: '1.3rem' }}>{team.flag}</span>
      <span className={['truncate leading-tight', isWinner ? 'text-amber-400 font-bold' : 'text-white/95'].join(' ')}
  ...
  return (
    <div className="absolute left-0" style={{ top, width: NODE_W, height: mh }}>
      <div className={[
        ...
        isFinal ? 'border-amber-500/50 bg-white/[0.10]' : 'border-white/15 bg-white/[0.08]',
      ].join(' ')}>
        ...
        <div className="w-full border-t border-white/10" style={{ height: 2 }} />
...
function Connections({ rounds, positions }: { rounds: RoundData[]; positions: number[][] }) {
  ...
  return (
    <svg className="absolute inset-0 pointer-events-none" style={{ width: svgW, height: svgH, zIndex: 0 }}>
      {lines.map(l => (
        <path key={l.key} d={l.d} fill="none" stroke="rgba(148,163,184,0.45)" strokeWidth="1.2"
  ...
  return (
    <div className="absolute flex items-center justify-center" style={{ left: x, top, width: CHAMPION_W, height: CHAMPION_H }}>
      <div className="text-center px-4 py-3 rounded-2xl w-full"
        style={{
          background: 'linear-gradient(160deg, rgba(251,191,36,0.25) 0%, rgba(245,158,11,0.12) 60%, rgba(255,255,255,0.03) 100%)',
          border: '1.5px solid rgba(251,191,36,0.55)',
          boxShadow: '0 0 50px rgba(251,191,36,0.25), 0 0 100px rgba(251,191,36,0.12)',
        }}>
        ...
        <div className="text-[10px] text-white/50">{champion.nameEn}</div>
      </div>
    </div>
  );
}
...
export default function BracketTree({
  ...
  return (
    <div className="relative">
        ...
        <div className="relative"
          ...
          {rounds.map((round, roundIdx) => (
            <div key={roundIdx} className="absolute" style={{ left: roundIdx * (NODE_W + COL_GAP), top: 0, width: NODE_W }}>
              <div className="absolute text-center w-full font-bold text-white/60 tracking-wider whitespace-nowrap"
          ...
          {champion
            ...
            : (
              <div className="absolute flex items-center justify-center opacity-25"
                style={{ left: rounds.length * (NODE_W + COL_GAP),
                  top: champCenterY - CHAMPION_H / 2, width: CHAMPION_W, height: CHAMPION_H }}>
                <div className="text-center"><div className="text-4xl mb-1">🏆</div><div className="text-[10px] text-white/50">等待冠军</div></div>
              </div>
            )}
        </div>
      </div>
      ...
      {totalFilled < 5 && (
        <div className="absolute left-1/2 -translate-x-1/2 -bottom-1 text-[11px] text-white/40 pointer-events-none">← 滑动查看 →</div>
      )}
    </div>
  );
}
